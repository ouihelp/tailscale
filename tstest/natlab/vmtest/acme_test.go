...
func TestACMECertServeHTTPS(t *testing.T) {
	...
	out, err = env.SSHExec(client, "printf %s "+shellQuote(rootB64)+" | base64 -d >/tmp/fake-acme-root.pem")
	if err != nil {
		t.Fatalf("install fake ACME root: %v\n%s", err, out)
	}
	out, err = env.SSHExec(client, "curl --fail --silent --show-error --cacert /tmp/fake-acme-root.pem https://"+shellQuote(domain)+"/")
	if err != nil {
