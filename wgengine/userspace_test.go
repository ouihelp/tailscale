...
func TestUserspaceEnginePeerCountMetrics(t *testing.T) {
	...
	for _, want := range []int{3, 1, 0} {
		...
		// No traffic flows in the fake engine, so wireguard-go never
		// instantiates a peer; active must stay within [0, available].
		if got := metricNumPeersActive.Value(); got < 0 || got > int64(want) {
			t.Errorf("wgengine_peers_active = %d, want in [0,%d]", got, want)
		}
	}
}
